...
namespace Kernel
{
    ...
    extern "C" __attribute__((noreturn)) void entry(uint32_t magic, multiboot_info_t *multiboot_info)
    {
        ...
        for (uint32_t i = 0; i < sizeof(heapTestArray) / sizeof(void *); i++)
            heapTestArray[i] = kmalloc(7 * (i + 1));

        auto stats __attribute__((unused)) = Kernel::Heap::getStatistics();

        for (auto ptr : heapTestArray)
            kfree(ptr);
